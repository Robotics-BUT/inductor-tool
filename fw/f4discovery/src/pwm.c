...
uint32_t pwm[4] = { 1, 0x0FF, 0x3FF, 1};
...
void pwm_init(void)
{
	...
	gpio_set_output_options(GPIOA, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ,  GPIO1 | GPIO2 | GPIO3);

	timer_reset(TIM5);
    timer_set_mode(TIM5, TIM_CR1_CKD_CK_INT, TIM_CR1_CMS_EDGE, TIM_CR1_DIR_UP);
    timer_set_prescaler(TIM5, 0);   // 20.5kHz
	...
	timer_set_oc_polarity_high(TIM5, TIM_OC1);      // CCxP
